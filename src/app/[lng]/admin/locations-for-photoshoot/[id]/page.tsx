...
import {
    ...
    setCreatingStatus, setUpdatingStatus, updateLocation
...
const AddNewPage = ({params: {id}}: Params) => {
    ...
    const creatingStatus = useAppSelector((state) => state.Locations.creatingStatus);
    const updateStatus = useAppSelector((state) => state.Locations.editingStatus);
    const fetchCurrentStatus = useAppSelector((state) => state.Locations.fetchOneLocationStatus);
    ...
    useEffect(() => {
        if (id !== "new") {
            dispatch(fetchOneLocation(parseInt(id)));
        }
        return () => {
            if (id !== "new") {
    ...
    useEffect(() => {
        ...
        setValue(`price`, currentLocation?.price);
        setValue(`date`, currentLocation?.date);
        setPreviewPhoto(currentLocation?.image as string);
    }, [currentLocation]);
    ...
    } = useForm<Partial<LocationPhotoshoot>>({
        defaultValues: {
            width: 268,
            height: 310
        }
    ...
    const fileInputLabel = fileInputValue && fileInputValue.name ? fileInputValue.name
        //@ts-ignore
        : id !== "new" && locations.length && locations.find(item => item.id == id) ? locations.find(item => item.id == id)?.image_name : 'Select file'
    ...
    const submitHandler = (data: any) => {
        ...
        if (id === "new") {
            ...
            formData.append("date", data.date);
            if (fileInputValue) formData.append("image", fileInputValue);
            dispatch(updateLocation({data: formData, id: parseInt(id)}))
                .then((res) => {
                    if (res.meta.requestStatus === "fulfilled") {
                        reset();
                        setFile(undefined);
                ...
                <form className={styles.form} onSubmit={handleSubmit(submitHandler)}>
                    <div className={styles.content}>
                        <div className={styles.left}>
                            ...
                            <div className={styles.inputWrapper}>
                                <label className={`${styles.label}`}>Start and End Dates</label>
                                <input placeholder={"Enter range of dates"} {...register("date", {required: true})}
                                       className={styles.input} type="text"/>
                            </div>
                        </div>
